...
print("^3HighLife proudly powered by RageUI - https://github.com/iTexZoz/RageUI^0")
...
function RageUI.IsAnyMenuVisible()
			...
			if RageUI.CurrentMenu ~= nil then
				if RageUI.CurrentMenu.Controls.Back.Enabled and RageUI.CurrentMenu.Closable then
					if RageUI.CurrentMenu.Controls.Back.Pressed then
						...
						RageUI.PlaySound(RageUI.Settings.Audio[RageUI.Settings.Audio.Use].Back.audioName, RageUI.Settings.Audio[RageUI.Settings.Audio.Use].Back.audioRef)
...
function IsMenuVisibleTable(menuTable)
	for menuIndex=1, #menuTable do
		if RageUI.Visible(menuTable[menuIndex]) then
			return true
		end
	end
...
function RageUI.IsVisible(menu, header, glare, instructional, items, panels)
	if type(menu) == 'table' and IsMenuVisibleTable(menu) or RageUI.Visible(menu) then
		if header == true then
