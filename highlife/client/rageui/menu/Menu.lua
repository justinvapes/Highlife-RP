...
local menuWidthModifier = nil

function GetMenuWidthFromResolution()
    local finalRes = 1320

    -- local currentRes = GetActiveScreenResolution()

    local aspectRatio = GetAspectRatio()

    if aspectRatio >= 2.37 then
        finalRes = 856
    end
...
function RageUI.CreateMenu(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    ...
    Menu.X = X or GetMenuWidthFromResolution()
    Menu.Y = Y or 0
    Menu.Parent = nil
