...
CreateThread(function()
	...
	while exports.highlife == nil do
		Wait(100)
	end
	
	for k,v in pairs(exports.highlife:GetBaseConfig('Dealerships')) do
		if v.vehicles ~= nil then
			for j,vehicle in pairs(v.vehicles) do
				for i=1, #vehicle do
					vehicle_prices[GetHashKey(vehicle[i].model)] = vehicle[i].price
				end
			end
		end
	end
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	PlayerRank = xPlayer.job.grade
end)

RegisterNetEvent('esx:setJob')
AddEventHandler('esx:setJob', function(job)
	PlayerJob = job
	PlayerRank = job.grade
end)

function isMechanic()
	if PlayerJob ~= nil and PlayerJob.name == 'mecano' then
		PlayerRank = PlayerJob.grade 
...
local garages = { 
	[1] = {
		locked = false,
		outside = {x = -333.655,y = -136.55, z = 38.295, heading = 71.187133},
		-- camera = {x = -330.945, y = -135.471, z = 39.01, heading = 102.213}, 
		-- driveout = {x = -350.376,y = -136.76, z = 38.294, heading = 70.226},
		-- drivein = {x = -333.655,y = -136.55, z = 38.295, heading = 249.532},
		-- inside = {x = -337.3863,y = -136.9247,z = 38.5737, heading = 269.455}
	},
	[2] = {
		locked = false,
		outside = {x = 733.09, y = -1081.721, z = 22.169, heading = 89.248},
	...
	[5]= {
		isSpecial = true,
		locked = false, 
		outside = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5},
		-- camera = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}, 
		-- driveout = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5},
		-- drivein = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}, 
		-- inside = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}
	},
	[6]= {
		isSpecial = true,
		locked = false, 
		outside = {x = -82.09 ,y = -822.13, z = 284.58, heading = 91.5},
		-- camera = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}, 
		-- driveout = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5},
		-- drivein = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}, 
		-- inside = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}
	...
	[7]= {
		isSpecial = true,
		locked = false, 
		outside = {x = -80.08 ,y = -816.73, z = 284.58, heading = 91.5},
		-- camera = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}, 
		-- driveout = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5},
		-- drivein = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}, 
		-- inside = {x = -222.17,y = -1330.06, z = 30.47, heading = 91.5}
...
-- { ['x'] = 110.1402130127, ['y'] = 6615.765625, ['z'] = 31.336744308472 },
-- { ['x'] = 104.12939453125, ['y'] = 6622.3803710938, ['z'] = 31.311761856079 },
...
function myveh.repair()
	-- local InitFuelLevel = GetVehicleFuelLevel(myveh.vehicle) * 1.0

	-- SetVehicleFixed(myveh.vehicle)

	-- exports.highlife:SetVehicleFuel(myveh.vehicle, InitFuelLevel)
end
...
local vehiclePartModifier = .00008
...
local function AddMod(mod, parent, header, name, info, stock)
	...
	local vehiclePrice = vehicle_prices[GetEntityModel(veh)] or 20000

	if vehiclePrice > 700000 then
		vehiclePrice = 700000
	end
	...
	if (GetNumVehicleMods(veh,mod) ~= nil and GetNumVehicleMods(veh,mod) > 0) or mod == 18 or mod == 22 then
		...
		if LSC_Config.prices.mods[mod].startprice then
			...
			for i=0, tonumber(GetNumVehicleMods(veh,mod)) -1 do
				...
				if lbl ~= nil then
					local mname = (GetLabelText(lbl) ~= 'NULL' and GetLabelText(lbl) or 'Part ' .. i)
					
					if mname ~= "NULL" then
						local chosenPartPrice = (price * (i + 1)) * (vehiclePrice * vehiclePartModifier)

						local btn = m:addPurchase(mname, math.floor(chosenPartPrice))
			...
			for n, v in pairs(LSC_Config.prices.mods[mod]) do
				local chosenPartPrice = (v.price * 1) * (vehiclePrice * vehiclePartModifier)

				btn = m:addPurchase(v.name, math.floor(chosenPartPrice)) --v.price)
				btn.modtype = mod
				btn.mod = v.mod
			end
		end
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		local price = round(250+150*damage,0)
		...
		function loadDefaults()
			...
			if not IsThisModelABike(GetEntityModel(veh)) and isMechanic() then
			...
			if isMechanic() then
				if IsVehicleSprayable(veh) then
					...
					if IsThisModelABike(GetEntityModel(veh)) then
						...
						if currentSpecial then
							if PlayerRank > 2 then
								-- customw = wtype:addSubMenu("CUSTOM WHEELS", "Custom", nil,true)
								-- 	for n, w in pairs(LSC_Config.prices.custom_wheels) do
								-- 		local btn = customw:addPurchase(w.name .. ' #' .. w.mod,w.price)btn.wtype = w.wtype btn.modtype = 23 btn.mod = w.mod
								-- 	end
							end
						end
		...
		if currentSpecial then
			if PlayerRank >= 3 then -- Bodywork Specialist
				...
				if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)))  then
					...
					for i,pos in ipairs(garages) do
						...
						if LSC_Config.oldenter then			
							...
							if Vdist(outside.x,outside.y,outside.z, GetEntityCoords(ped)) <= 10.0 then
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.isSpecial then
										currentSpecial = true
										-- print('current special is true')
									else
										currentSpecial = false
										-- print('current special is false')
									end
											...
											if pos.locked then
												...
												if not LSC_Config.lock then
													if IsControlJustPressed(1,201) then
														...
														drawTxt("Press ~y~ENTER~w~ to modify vehicle",4,1,0.5,0.8,1.0,255,255,255,255)
													end
												else
										...
										if pos.locked then
											...
											if not LSC_Config.lock then
												if IsControlJustPressed(1,201) then
													...
													drawTxt("Press ~y~ENTER~w~ to visit ~b~LS Customs",4,1,0.5,0.8,1.0,255,255,255,255)
												end
											else
												drawTxt("~r~Locked, please wait",4,1,0.5,0.8,1.0,255,255,255,255)
											end
											...
											if IsControlJustPressed(1,201) then
												...
												drawTxt("Press ~y~ENTER~w~ to visit ~p~LS Customs",4,1,0.5,0.8,1.0,255,255,255,255)
											end
										end
									end
								else
									drawTxt("~r~This vehicle can't be modified",4,1,0.5,0.8,1.0,255,255,255,255)
								end
							end
						else
...
function LSCMenu:onButtonSelected(name, button)
	...
	if button.price then
		TriggerServerEvent("LSC:buttonSelected", name, button, (GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(PlayerPedId())))) ~= 'NULL' and GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(PlayerPedId())))) or GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(PlayerPedId())))), GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId())))
	end
end
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	elseif mname == "custom" or mname == "sport" or mname == "muscle" or mname == "lowrider" or mname == "back wheel" or mname == "front wheel" or mname == "highend" or mname == "suv" or mname == "offroad" or mname == "tuner" then
		if button.purchased or CanPurchase(price, canpurchase) then
...
function LSCMenu:OnMenuChange(last,current)
	...
	elseif c == "front wheel" or c == "wheel accessories" or  c == "wheel color" or c == "sport" or c == "custom" or c == "muscle" or c == "lowrider"  or c == "highend" or c == "suv" or c == "offroad" or c == "tuner" then
...
function CheckPurchases(m)
	...
	elseif name == "sport" or name == "custom" or name == "muscle" or name == "lowrider" or name == "back wheel" or name == "front wheel" or name == "highend" or name == "suv" or name == "offroad" or name == "tuner" then
		for i,b in pairs(m.buttons) do
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			...
			-- TriggerEvent('eden_garage:saveVeh')
		end
		return true
	else
