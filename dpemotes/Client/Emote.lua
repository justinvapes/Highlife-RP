...
Citizen.CreateThread(function()
  while true do
    ...
    if Config.MenuKeybindEnabled then 
      if IsControlPressed(0, Config.MenuKeybind) and GetLastInputMethod(2) then 
        if not exports.highlife:InAmbulance() and not exports.highlife:IsPlayingBlackjack() and not exports.highlife:GetHandCuffStatus() and not exports.highlife:GetIsInDetention() and not IsHighLifeGradeDead(PlayerPedId(), true) then 
          OpenEmoteMenu()
        end
      end 
    end
...
function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
  ...
  CreateObject(modelHash, x, y, z, isNetwork, thisScriptCheck, dynamic)

  prop = CreateObject(GetHashKey(prop1), GetEntityCoords(PlayerPedId()), true, false, false)
...
RegisterNetEvent("dp:playanim") -- For opening the emote menu from another resource.
AddEventHandler("dp:playanim", function(name)
    if name == 'cancel' then
      EmoteCancel()
    else
      if DP.Emotes[name] ~= nil then
