...
mainMenu = NativeUI.CreateMenu("Emotes", "", menuPosition["x"], menuPosition["y"])
...
Citizen.CreateThread(function()
  while true do
    if mainMenu:Visible() and IsHighLifeGradeDead(PlayerPedId()) then
      mainMenu:Visible(false)
    end

    if Config.FavKeybindEnabled then
      if IsControlPressed(0, Config.FavKeybind) then
        if not exports.highlife:InAmbulance() and not exports.highlife:GetHandCuffStatus() and not exports.highlife:GetIsInDetention() and not IsHighLifeGradeDead(PlayerPedId()) and not exports.highlife:CoreControlsBlocked() then
...
function AddWalkMenu(menu)
    ...
    submenu.OnItemSelect = function(sender, item, index)
      if item ~= walkreset then
        ...
        TriggerServerEvent('HAnimations:setWalkStyle', WalkTable[index])
      else
        WalkMenuStart('Reset')

        TriggerServerEvent('HAnimations:setWalkStyle', (isMale() and 'move_m@multiplayer' or 'move_f@multiplayer'))
      end
    end
end
